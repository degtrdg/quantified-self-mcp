...
def _get_all_tables_overview() -> str:
        ...
        for table_row in basic_tables:
            ...
            try:
                ...
                table_info = {
                    ...
                    if metadata and len(metadata) > 0 and metadata[0]["description"]
                    else "No description",
                    "purpose": metadata[0]["purpose"]
                    if metadata and len(metadata) > 0 and metadata[0]["purpose"]
                    else "No purpose defined",
                    "ai_learnings": "{}",  # Empty for now since ai_learnings column doesn't exist
                    "column_count": column_info[0]["column_count"]
    ...
    for table in tables:
        ai_learnings = (
            json.loads(table["ai_learnings"]) if table["ai_learnings"] else {}
        )
...
def _get_detailed_table_info(table_name: str) -> str:
    ...
    for col in columns:
        ...
        result.append(
            f"| {col['column_name']} | {col['data_type']} | {nullable} | {default} | {col['description']} | {units} |"
        )
...
def view_table(table_name: str, limit: int = 3) -> str:
    ...
    try:
        ...
        recent_data = db_helper.execute_query(
            f"SELECT * FROM {table_name} ORDER BY created_at DESC LIMIT %s", (limit,)
        )

        oldest_data = db_helper.execute_query(
            f"SELECT * FROM {table_name} ORDER BY created_at ASC LIMIT %s", (limit,)
        )
