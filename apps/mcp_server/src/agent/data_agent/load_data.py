...
class DataLoader:
    ...
    def load_table(
        self, table_name: str, parse_dates: bool = True
        ...
        try:
            if parse_dates:
                ...
                existing_date_cols = [
                    col for col in date_columns if col in df_preview.columns
                ]
...
"""
            ...
            info = [
                f"📊 **Data Overview**: {table_count} tables, {total_rows:,} total rows\n"
            ]

            for _, row in self.metadata.iterrows():
                info.append(
                    f"• **{row['table_name']}**: {row['row_count']:,} rows - {row['description']}"
                )
